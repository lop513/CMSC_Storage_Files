...
// Built-in Shell fucntion declerations
int shell_exit(char **args);

int shell_proc(char **args);

char *builtin_commands[] = { "exit" , "proc"};

int (*builtin_functions[]) (char **) = { &shell_exit , &shell_proc};
...
int shell_proc(char **args){
  ...
  if(arg_size == 2){
    ...
    strcpy(file_string, "/proc/");
    strcpy(second_arg, args[1]);
  
    strcat(file_string, second_arg);
    
    if((fptr = fopen(file_string, "r")) == NULL){

      fprintf(stderr, "Simple Shell: Cannot Open File \n");
    ...
    fprintf(stderr, "Simple Shell: Cannot execute proc command, takes only two  arguments \n");
  }
